...
describe('PATCH /api/organizations/[id]/owner', () => {
  beforeEach(() => {
    vi.resetAllMocks()
  })
  ...
  it('should return 403 when role is employee', async () => {
    ...
    const req = new NextRequest('http://localhost/api/organizations/org-1/owner', {
      body: JSON.stringify({ ownerId: 'emp-2' }),
    })
  ...
  it('should return 200 on success', async () => {
    ...
    const updated = { id: 'org-1', ownerId: 'emp-2', owner: { id: 'emp-2', fullName: 'Bob' } }
    ...
    expect(mockDbUpdate).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ ownerId: 'emp-2' }) }),
    )
  })
})
