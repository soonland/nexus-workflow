...
describe('POST /api/organizations/[id]/reactivate', () => {
  ...
  it('should return 403 when role is employee', async () => {
    ...
    const req = new NextRequest('http://localhost/api/organizations/org-1/reactivate')
  ...
  it('should return 200 and set status to active', async () => {
    mockAuth.mockResolvedValue({ user: { id: 'user-1', role: 'manager' } })
    mockDbFindUnique.mockResolvedValue({ id: 'org-1' })
    const updated = { id: 'org-1', status: 'active', owner: null }
